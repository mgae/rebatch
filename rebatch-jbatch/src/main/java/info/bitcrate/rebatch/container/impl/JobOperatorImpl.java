...
import info.bitcrate.rebatch.container.Init;
...
import info.bitcrate.rebatch.jmx.Rebatch;
import info.bitcrate.rebatch.jmx.RebatchMBean;
import info.bitcrate.rebatch.jmx.RebatchMBeanImpl;
...
import static info.bitcrate.rebatch.container.util.ClassLoaderAwareHandler.makeLoaderAware;
...
public class JobOperatorImpl implements JobOperator {
    static {
        ...
        if (Boolean.parseBoolean(ServicesManager.value("info.bitcrate.rebatch.jmx", "true"))) {
            try {
                final MBeanServer platformMBeanServer = ManagementFactory.getPlatformMBeanServer();
                final String app = ServicesManager.value("info.bitcrate.rebatch.jmx.application", "");
                final ObjectName name;
                if (app.isEmpty()) {
                    name = new ObjectName(RebatchMBean.DEFAULT_OBJECT_NAME);
                } else {
                    name = new ObjectName(RebatchMBean.DEFAULT_OBJECT_NAME + ",application=" + app);
                }
                ...
                platformMBeanServer.registerMBean(
                    new Rebatch(
                        makeLoaderAware(RebatchMBean.class, new Class<?>[]{ RebatchMBean.class }, RebatchMBeanImpl.INSTANCE)),
                    name);
